...
void dial::timeSetter::loadFontBtn()
{
    ...
    this->home_back_btn->SetFillColor(sf::Color(TIMER_TEXT_COLOR));
    this->home_back_btn->SetTextColor(sf::Color::Black);
...
void dial::timeSetter::initialSetup(sf::RenderWindow &window, int a)
{
    ...
    if (a == 1)
    {
        ...
        this->updatePoints(this->stopTime);
    }
    if (a == 2)
    {
        this->pauseSound.play();
        this->updatePoints(-10.0f);
    }
...
std::vector<sf::Vector3f> dial::timeSetter::findEachMinpos()
{
    ...
    for (float y = b - r; y < b + r; y += 0.1f)
    {
        ...
        if (five_pos1.size() > 1)
        {
            ...
            //when angle is 90 or 270 then extra values are added like 30 31 30 31
            if (time == 30.0f || time == 31.0f)
            {
                if (five_pos1[five_pos1.size() - 3].x == time)
                {
                    five_pos1.pop_back();
                }
            }
    ...
    for (float y = b + r; y >= b - r; y -= 0.1f)
    {
        ...
        if (five_pos1.size() > 1)
        {
            if (five_pos1[five_pos1.size() - 2].x == time || five_pos1[five_pos1.size() - 3].x == time)
            {
                five_pos1.pop_back();
            }
        }
...
void dial::timeSetter::pointKnob()
{
    ...
    if (this->stopTime < 25.0f)
    { //not letting user set time < 25 mins
        this->stopTime = 25.0f;
    }
    this->stop_pos = this->posForTime(this->stopTime);
}
...
void dial::timeSetter::printPressing(sf::RenderWindow &window)
{
    ...
    this->knobCircle.setPosition(sf::Vector2f(this->mainCircle.getPosition().x, this->mainCircle.getPosition().y - this->mainCircle.getRadius()));
...
void dial::timeSetter::printTicking(sf::RenderWindow &window, int &tickingFlag, int &stopPressed)
{
    ...
    if (stopPressed != 2)
    {
        ...
        //updating fillers
        if (min)
        { //to not print the filler when time becomes zero
            for (auto it = all_pos.begin(); it != all_pos.end(); ++it)
            {
                fillers.setPosition(*it);
                window.draw(fillers);
...
void dial::timeSetter::dialPollEvents(sf::RenderWindow &window, sf::Event &event)
{
    ...
    if (this->tickingFlag == 2)
        timerTicking = 1; //timerTicking in toogleMusic.h
    if (this->stopPressed == 2)
        timerTicking = 0;

    if (toggleStatus)
    {
        if (this->tickingFlag == 2)
        { //if play button is pressed music plays
            if (!isPlaying)
            {
                playActiveMusic();
            }
        ...
        if (this->stopPressed)
        {                     //if stop button is pressed music stops
